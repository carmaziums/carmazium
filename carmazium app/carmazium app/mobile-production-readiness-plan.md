...
**Status: DONE** (2026-07-16). Ported `carData.ts` verbatim to `src/data/carData.ts` (including `VARIANTS_BY_MODEL`/`getVariantsForModel`, not currently wired to anything on mobile — kept for parity and in case a Variant dropdown is requested later, since the reported bug only asked for Make/Model). Built a new `PickerField` component (`SellCarFlowScreen.tsx`, alongside `FieldInput`) rather than a literal port of web's HTML `<select>` + `"__other__"` sentinel pattern — a `BottomSheet` with a search box, filtered option list, and an always-available "Use '<query>'" row lets the user pick from the list or type anything, adapted for touch/search UX rather than a raw select element. Selecting a new Make clears the current Model (its option list depends on Make); Model's picker falls back to a pure free-text sheet (empty option list) when no Make is set yet or the selected Make has no known models, matching web's fallback behavior. No backend change — `make`/`model` remain plain strings server-side, so anything typed via the "Use '<query>'" row round-trips exactly like the old free-text fields did.
...
## Phase 5 — Second on-device round (2026-07-18)

After Stage 24-26 shipped and a fresh release APK was built, the user tested again and reported 6 more findings. Two of these were re-reports of items believed already fixed (F35 auction bidding — no, sorry, F38 dealer inventory, and F35 notification navigation) — both were re-verified end-to-end against current source and found correct, with the fix confirmed present in the compiled bytecode of the built APK, so these are very likely a stale build on the test device rather than a code gap; flagged to the user rather than silently re-"fixed." The other four were genuine new findings, investigated before any change:

### F40 — KYC required again when a previously-verified user switches back to dealer role

Real bug, root-caused precisely. `DealerProfile`/`DealerKyc` records are one-per-user and persist forever — `backend/src/users/users.service.ts`'s `requestRoleElevation` (`POST /users/elevate`) only ever updates the `User.role` column, never touches `DealerProfile`/`DealerKyc`. Web's dealer-dashboard gate (`src/app/dashboard/dealer/layout.tsx`) correctly checks `dealerProfile?.isVerified` alone, independent of current role — this is the proven-correct parity reference.

Mobile's bug: `GlobalDrawer.tsx`'s "Become a Dealer" / "Switch to Dealer Mode" toggle computed `isActualDealer = role === 'dealer'` — the **mutable buyer-preview role**, not the real account state — and the whole toggle block only renders when `role !== 'dealer'` in the first place, meaning `isActualDealer` was **always false inside its own branch**, so the `isActualDealer && user?.isVerified` shortcut (meant to skip straight to dealer view) could never fire. Every previously-verified user who'd switched away from dealer view fell through to the final `else`, sending them to `DealerOnboardingScreen` — a full form (trading name, VAT, etc.) — every single time.

**Status: DONE.** Two-layer fix: (1) `GlobalDrawer.tsx` now checks `user?.isVerified` *first*, independent of role, matching web's gate exactly — if verified, it silently re-elevates via `/users/elevate` (only if the backend role isn't already DEALER) and switches straight to dealer view, no form, with a loading spinner on the toggle row during the elevate call. `isActualDealer` is now based on `accountRole` (the real account role, not the preview toggle) for the remaining not-yet-verified branch. (2) Added a self-contained guard directly inside `DealerOnboardingScreen.tsx` (not just the one drawer entry point) — on mount, if `user?.isVerified` is true, it skips the form entirely, shows a brief "already verified, switching you over" loading state, silently re-elevates if needed, and navigates back. This covers the two *other* unconditional entry points found (`HomeScreen.tsx`'s dealer CTA, `PricingScreen.tsx`'s dealer CTA) without needing to patch each one individually, and protects against any future entry point too.

### F41 — 3D model doesn't render for buyers, and breaks when navigating back and forth through listing-creation steps

Real bug, two compounding causes found in `ThreeDVehicleViewer.tsx` and `SellCarFlowScreen.tsx`:

1. **Full teardown-and-rebuild on every step AND every photo-tab change.** `SellCarFlowScreen.tsx`'s step rendering (`{step === 2 && renderStep2()}`) is a plain conditional, not a hide/show toggle — moving away from step 2 (or even just switching the Exterior/Interior/Damage tab *within* step 2) fully unmounts `Damage3DMapper`/`ThreeDVehicleViewer`. `ThreeDVehicleViewer`'s own `useEffect` (empty dependency array, so it reruns from scratch on every fresh mount) re-downloads and re-reads the HTML template, a ~744KB Three.js bundle, and re-base64-encodes a ~2.4MB GLB file, then creates a brand-new native WebView instance and re-injects everything via `injectJavaScript`. Repeated rapid remounts (exactly what "going back and forth between steps" produces) is a known failure mode for RN WebView — blank screens, dropped WebGL context.
2. **Genuinely silent failure path.** `handleWebViewLoad`'s catch block was empty except for a comment claiming "the viewer.html error overlay handles the visible failure state" — true only if the injection JS actually ran. If the RN-side asset resolution/read throws *before* `injectJavaScript` fires, `viewer.html`'s own error UI never activates either, leaving the user staring at a permanent "Loading 3D model..." with zero error signal.
3. **Buyer-side error swallowing.** `VehicleDetailScreen.tsx`/`AuctionDetailScreen.tsx` fetch `/damage/{listingId}` with a bare `.catch(() => {})` — any fetch failure (network, auth, 5xx) leaves `damageRecords` empty exactly like a real "no damage recorded" listing, and `BuyerDamageViewer.tsx` never even mounts the 3D viewer when records are empty, regardless of *why* they're empty. This is visually indistinguishable from "the 3D model isn't initializing."

**Status: DONE.** (1) Cached the expensive HTML+bundle+GLB asset resolution/read/base64-encode chain at module scope in `ThreeDVehicleViewer.tsx` (a promise cached outside the component, resolved once per app session) — repeated mounts across step/tab navigation now reuse the already-loaded data instead of redoing multi-MB disk I/O and encoding every time; the WebView itself still remounts (unavoidable without a larger wizard-architecture change, out of scope here) but the reload is now fast and doesn't re-touch the filesystem. (2) `handleWebViewLoad`'s catch now sets a visible error state with a manual retry button instead of failing silently. (3) `VehicleDetailScreen.tsx`/`AuctionDetailScreen.tsx` now distinguish a real fetch failure from a genuine empty-damage-list, and `BuyerDamageViewer.tsx` shows a distinct "couldn't load damage info, tap to retry" state instead of silently rendering the clean-vehicle empty state on a network error.

### F42 — Locate-me button diagnosability

Investigated first: no logic bug found in the happy path (permission handling, `postcodes.io` query params, and the generated `AndroidManifest.xml`'s `ACCESS_FINE_LOCATION`/`ACCESS_COARSE_LOCATION` entries were all confirmed correct). But the error handling was bad enough to make the report impossible to diagnose further: a bare, unlogged `catch {}`, no `res.ok` check before parsing the postcodes.io response (a non-2xx error body would parse fine and silently masquerade as "no postcode found" rather than "request failed"), and no timeout on `getCurrentPositionAsync` (a device with Location Services off at the OS level, or no Play Services on an emulator, could hang or reject with no way to tell which).

**Status: DONE.** Bound and logged the error in every catch path, added an explicit `res.ok` check before parsing, added a 10s timeout to the location request with a distinct "timed out" message, and split out device-location-services-disabled as its own distinct, correctly-worded alert rather than falling into the generic failure message.

### F43 — "Won" auctions category for dealers (new feature, not a parity gap)

Not a web parity miss — web's own dealer auctions page (`src/app/dashboard/dealer/auctions/page.tsx`) has no "Won" tab either, it's `sellerId`-scoped exactly like mobile's `SellerAuctionsScreen.tsx`. Won-auction purchases already land in `DealerPurchasesScreen.tsx` (`Sale` rows are created with `buyerId: winnerId` the moment an auction closes with a winner), but that list is generic (mixes auction wins with direct classified/BIN purchases, no auction-specific fields like winning bid or bid count) and isn't linked from the Auctions tab at all.

**Status: DONE.** Added `GET /auctions/my/won` on the backend (`auction.findMany({ where: { winnerId: userId } })`, same response shape as the existing `my/list` endpoint) and a new "WON" tab in `SellerAuctionsScreen.tsx`'s filter row, giving real auction-specific data (winning bid, end time, seller) rather than reusing the generic post-sale Purchases data.

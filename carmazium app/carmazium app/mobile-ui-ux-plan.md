...
> - **Stage 3 (BottomSheet migration, C2) — MOSTLY DONE.** 13 files now use `BottomSheet` (up from 2). **Still open:** `SellerAuctionsScreen.tsx`'s schedule-auction modal (called out in the original plan as "biggest single win," ~250 lines) is still a hand-rolled `<Modal>`. `VehicleDetailScreen.tsx` also still has `<Modal>` usage — not yet checked whether that's the HPI/thumbnail/offer modals flagged in the plan or something already-acceptable.

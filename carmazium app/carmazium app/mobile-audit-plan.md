...
> - **Stage 3 (W3, W4, W7) — DONE as of 2026-07-12.** The defensive null-guard in `DamageMapViewer.tsx` (a `validRecords` filter checking `coords`/`x`/`y`/`view` before render) was already in place — W4's crash risk was mitigated. The remaining half of W4 (the actual zone-id diff against web) is now done too: all 33 ids in `damageZones.ts` were diffed programmatically against the web repo's authoritative `ALL_ZONES` (`src/components/listing/ThreeDVehicleViewer.tsx`) — **14 of 33 were wrong** (not "23 unverified" as this file's old comment worried; most of the 23 flagged-as-unverified ones turned out already correct) and have been corrected. See `mobile-production-readiness-plan.md` Section 1 for the full list of corrections. W3 (interior zone coordinate spread) was independently confirmed already fixed — the 8 interior zones each have distinct `coords.y` values (10, 21, 32, ... 87), not stacked.

...
> Note on branching: this branch is cut from `main`, so it does not contain Flow 1's entry
> above it. Every flow branching off `main` means each one appends to the same two docs from
> the same base, and they will conflict on merge. Either merge each flow into `main` once
> verified before cutting the next, or cut each flow from the previous one.

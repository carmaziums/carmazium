...
- **Status (23 Sep 2026): provider foundation + Jobs + Leads complete; Block 7 remains in progress.** Native mobile now has the Partner Account dashboard, service add-on application/status, Stripe Connect entry, secure provider verification evidence, Delivery/Inspection/Finance/Warranty matching settings, the full paid-job provider lifecycle, and the matched Finance/Warranty provider inbox. Provider Jobs covers matching feed, quoting, assigned work, customer contact, service-job chat, start/complete and structured inspection outcomes; Provider Leads covers matched enquiry list/detail plus create/update provider responses using the same backend contracts as web. `partner.dashboard`, `service_provider.capabilities`, `service_provider.jobs` and `service_provider.leads` are required parity surfaces. The provider-focused Messages workspace remains the final explicit Block 7 gap.

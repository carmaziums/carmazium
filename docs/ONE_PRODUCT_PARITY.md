...
- **Checkpoint (23 Sep 2026): reachability remediation in progress.** The remaining buyer auction-inspection/refusal gap is now implemented natively: a won auction can create/reopen its linked inspection, customers can reach My Service Jobs via navigation or deep link, compare/accept quotes, follow payment/job state, contact the provider, confirm/dispute/cancel work, and refuse the auction with a full £125 buyer-fee refund when a completed linked inspection records verified faults. Public vehicle and live-auction URLs now hydrate through id/slug wrapper screens and reset the native stack to Tabs → Detail so cold-start back navigation stays usable. Cross-role dashboard navigation now also preserves Tabs beneath authenticated deep-linked stack screens, every drawer stack target is CI-checked against MainStack registration, and Buyer Dashboard routes Search through the Tabs navigator instead of a dead top-level route. iOS associated-domain and Android HTTPS intent prerequisites are declared; production website association files still require the real Apple app identifier and Android signing certificate fingerprint before universal-link verification can be certified. Shared dealer/Partner terminology is now aligned and guarded across web/native navigation and screen titles. Loading/empty/error/offline consistency is now guarded across both clients: native keeps its app-wide connectivity monitor plus shared EmptyState/ErrorBanner/Skeleton primitives, while web now has an app-wide offline status and shared retryable LoadingState/EmptyState/ErrorState components used by representative dealer, unified buyer/seller and Partner/TradeXchange journeys. Accessibility hardening now includes visible keyboard focus, reduced-motion support, skip-to-content navigation, explicit menu/current-page semantics on web, and required accessible names for native icon-only controls; these invariants are CI-guarded. Static performance hardening now defers the non-critical Mazium chunk until browser idle, virtualizes the three image-heavy native onboarding slides, narrows bundled native assets, and enables Android release minification/resource shrinking behind the existing Expo/R8 keep rules; `ui.performance_baseline` guards these invariants. Runtime browser/screen-reader accessibility measurements, production Core Web Vitals, real-device memory/scroll checks and signed APK/AAB size measurements remain release-certification work rather than inferred claims.

- **Revalidation (25 Sep 2026): Block 9 correction pass.** The post-Block-8 audit found a functional role gap that the earlier reachability pass did not cover: web `FINANCE_PARTNER` and `INSURANCE_PARTNER` users had operational application/quote dashboards and integration settings, while native only preserved their identity on an informational account screen. Native now uses the same finance/insurance backend contracts for paginated work queues, approve/complete or quote/reject actions, company/callback settings and integration-key regeneration. Legacy `CONTRACTOR` accounts now land in the existing Partner workspace instead of the identity-only screen. The parity manifest also explicitly covers these role workspaces plus the direct-to-seller vehicle-payment boundary introduced in Block 8, and CI fails if native silently defaults an unspecified payment route into a CarMazium charge. The 24 Sep Block 10 checkpoint below is therefore historical and must be re-certified after this Block 9 change; Block 10 has not been run as part of this pass.
...
- **Completed (25 Sep 2026):** certification was re-run after the concurrent Registered Company / Sole Trader KYC merge rather than freezing the earlier Block 9 tree. The manifest now contains **54 required web/native features**, **2 approved web-only surfaces** and **0 unresolved gaps**. `dealer.kyc_business_type` explicitly guards the web/native business-type branch plus backend private-evidence and unpaid-draft safeguards. The current-main Vercel deployment (`cc568dec...`) was READY with clean build logs and no 4xx/5xx or warning/error groups in the checked first-hour sample. Block 10 adds a repository release-readiness script and a unified GitHub Release Certification workflow. Strict store-release certification remains intentionally blocked until real Apple/Android association/signing identifiers and physical-device evidence are supplied.

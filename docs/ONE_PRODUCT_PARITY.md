...
- **Completed (25 Sep 2026): certification/release gate established.** The revalidated manifest contains 54 required web/native features, including the registered-company/sole-trader KYC branch, 2 explicitly approved web-only surfaces and 0 unresolved gaps. The exact Block 9 `main` deployment was verified READY on Vercel; production route checks and runtime status counts were recorded in `docs/parity/BLOCK10_RELEASE_EVIDENCE.md`. `scripts/check-release-readiness.mjs` and `.github/workflows/release-certification.yml` now separate code-controlled certification from strict store-release evidence. The ten-block programme is complete, while the strict native store-release gate remains intentionally blocked until real Apple/Android association/signing identifiers and physical-device evidence are supplied.

...
- **Status (23 Sep 2026): Partner foundation + provider Jobs complete; Block 7 remains in progress.** Native mobile has the Partner Account dashboard, service add-on application/status, Stripe Connect entry, secure provider verification evidence and Delivery/Inspection/Finance/Warranty matching settings using the same backend contracts as web. Native provider Jobs now covers available/assigned work, quote/update/withdraw, paid-job start/complete, structured Inspection outcomes and service-job chat. `partner.dashboard`, `service_provider.capabilities` and `service_provider.jobs` are required parity surfaces. Native provider Leads and the provider-focused Messages workspace remain the explicit Block 7 gaps.

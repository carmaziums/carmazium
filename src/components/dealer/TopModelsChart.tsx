        ...
        <div className="dealer-glass-card overflow-hidden">
            ...
            {data.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                    ...
                    <p className="text-xs text-gray-600 font-bold">No sales data for this period</p>
                </div>
            ) : (
                    ...
                    {data.map((row, i) => {
                        ...
                        return (
                            <div key={`${row.make}-${row.model}`} className="px-5 py-3.5 flex items-center gap-4 hover:bg-white/[0.02] transition-colors group">
                                <span className={`text-xs font-black tabular-nums w-5 shrink-0 ${i === 0 ? 'text-amber-400' : i === 1 ? 'text-gray-300' : i === 2 ? 'text-amber-600' : 'text-gray-600'}`}>
                                    {i + 1}
                                </span>

    ...
    return (
        <div className="dealer-glass-card p-5 group relative overflow-hidden">
            ...
            <div className="relative z-10">
                <h3 className="text-3xl font-black font-heading tracking-tight leading-none" style={{ color: 'var(--text-primary)' }}>
                    {loading ? (
                        <span className="inline-block w-20 h-8 bg-[var(--bg-card)] rounded-lg animate-pulse" />
                    ) : (
                        <>
                            {prefix && <span className="text-lg text-[var(--text-muted)] font-bold mr-0.5">{prefix}</span>}
                            <span className="metallic-foil">{value}</span>
                            {suffix && <span className="text-base text-[var(--text-muted)] font-bold ml-1">{suffix}</span>}
                        </>
                    )}
                </h3>
                <div className="flex items-center justify-between mt-2">
                    <p className="text-[var(--text-muted)] text-xs uppercase tracking-[0.15em] font-bold">{label}</p>
                    {trendLabel && (
                        <span className="text-xs text-[var(--text-muted)] font-medium">{trendLabel}</span>
                    )}
                </div>
            </div>

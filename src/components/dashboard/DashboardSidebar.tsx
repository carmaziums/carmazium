...
import { useOptionalDealerAccess } from "@/context/DealerAccessContext"
...
export function DashboardSidebar({ role, userName: initialUserName, userType: initialUserType, children, myOffersCounterBadge }: SidebarProps) {
    ...
    const dealerAccess = useOptionalDealerAccess()
    ...
    const links: Record<string, LinkObj[]> = {
        ...
        dealer: DEALER_ROUTE_CONFIG
            .filter(route =>
                !route.hidden
                && (!route.permission || Boolean(dealerAccess?.can(route.permission)))
            )
            .map(route => ({
                href: route.href,
                label: route.label,

    ...
    return (
        <>
            ...
            >
                ...
                <span className="text-primary text-xs font-bold whitespace-nowrap group-hover:underline">Reveal number</span>
            </button>
            <LoginWall
                open={showLoginWall}
                onClose={() => setShowLoginWall(false)}
                redirectTo={pathname || '/'}
                message="Log in or create an account to view this seller's phone number."
            />
        </>
    )
}

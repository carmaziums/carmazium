...
export function EnquireModal({ listingId, dealerProfileId, isOpen, onClose, sellerName, carTitle }: EnquireModalProps) {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex justify-center overflow-y-auto p-4 sm:p-5" onClick={onClose}>
            <div className="glass-card p-6 sm:p-8 max-w-md w-full relative my-auto h-fit border border-[var(--border-default)] rounded-2xl" onClick={(e) => e.stopPropagation()}>
                <button 
                    onClick={onClose} 
                    className="absolute top-4 right-4 text-[var(--text-muted)] hover:text-primary dark:hover:text-white transition-colors disabled:opacity-50"
                >
                    <X size={24} />
                </button>
                ...
                <h2 className="text-2xl font-bold font-heading mb-2 text-center tracking-tight">
                ...
                <p className="text-[var(--text-muted)] mb-6 text-center text-sm">Send an enquiry and the seller will get back to you.</p>
                ...
                {status === "success" ? (
                    <div className="flex flex-col items-center justify-center py-8 text-center animate-in fade-in zoom-in duration-300">
                        ...
                        <p className="text-[var(--text-muted)]">The seller will get back to you shortly.</p>
                    </div>
                ) : (
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1">Your Name *</label>
                            <div className="relative">
                                <User className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)] w-5 h-5" />
                                <input 
                                    type="text" 
                                    required
                                    className="w-full bg-[var(--bg-input)] border border-[var(--border-default)] rounded-xl py-3 pl-10 pr-4 placeholder-[var(--text-muted)] focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all"
                        ...
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1">Phone Number (Optional)</label>
                            <div className="relative">
                                <Phone className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)] w-5 h-5" />
                                <input 
                                    type="tel" 
                                    className="w-full bg-[var(--bg-input)] border border-[var(--border-default)] rounded-xl py-3 pl-10 pr-4 placeholder-[var(--text-muted)] focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all"
                        ...
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1">Message</label>
                            <textarea 
                                className="w-full bg-[var(--bg-input)] border border-[var(--border-default)] rounded-xl py-3 px-4 placeholder-[var(--text-muted)] focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all h-24 resize-none"

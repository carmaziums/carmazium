        ...
        <main className="flex-1 max-w-4xl space-y-6">
            ...
            {data && <>
                ...
                <section className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-card)] p-6">
                    ...
                    {data.statusHistory.length === 0 ? <p className="text-sm text-[var(--text-muted)]">No status history recorded.</p> : <div className="space-y-3">{data.statusHistory.map(entry => <div key={entry.id} className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-input)] p-4"><div className="flex flex-wrap items-center gap-2"><span className="text-[10px] uppercase tracking-widest font-black text-[var(--text-muted)]">{entry.fromStatus || "NEW"}</span><span className="text-[var(--text-muted)]">→</span><span className="text-[10px] uppercase tracking-widest font-black text-primary">{entry.toStatus}</span></div>{entry.note && <p className="text-sm mt-2 whitespace-pre-line">{entry.note}</p>}<p className="text-[11px] text-[var(--text-muted)] mt-2">{entry.adminEmail || [entry.adminFirstName, entry.adminLastName].filter(Boolean).join(" ") || "System"} · {new Date(entry.createdAt).toLocaleString("en-GB")}</p></div>)}</div>}
                </section>

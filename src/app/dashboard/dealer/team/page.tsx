        ...
        <div className="min-h-screen pt-20 pb-12">
            <div className="container mx-auto px-5 flex flex-col lg:flex-row gap-8">
                ...
                <main className="flex-1 space-y-6 min-w-0">
                    ...
                    {/* Invite Form */}
                    {showInvite && (
                        <div className="dealer-glass-card p-8 animate-in fade-in slide-in-from-top-2 duration-300 bg-[var(--bg-card)] border-primary/20 ring-1 ring-primary/10">
                            <div className="flex items-center gap-3 mb-6">
                                <div className="p-2 bg-primary/10 rounded-lg border border-primary/20">
                                    <PlusCircle size={18} className="text-primary" />
                                </div>
                                <h3 className="font-black uppercase tracking-widest text-sm">Authorize New Personnel</h3>
                            </div>
                            <div className="flex flex-col lg:flex-row gap-4">
                                <Input
                                    ...
                                    className="bg-[var(--bg-input)] border-[var(--border-default)] placeholder:text-[var(--text-muted)] flex-1 h-12 rounded-xl focus:ring-1 focus:ring-primary/40"
                                />
                                <div className="relative group">
                                    <select
                                        value={inviteRole}
                                        onChange={e => setInviteRole(e.target.value)}
                                        className="bg-[var(--bg-input)] border border-[var(--border-default)] rounded-xl px-5 py-3 text-xs font-black uppercase tracking-widest h-12 min-w-[200px] appearance-none cursor-pointer focus:ring-1 focus:ring-primary/40"
                                    >
                    ...
                    {loading ? (
                        ...
                        <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-2xl p-16 text-center">
                            <Users className="h-14 w-14 text-gray-700 mx-auto mb-4" />
                            <p className="text-[var(--text-muted)] font-bold text-lg">No team members yet</p>
                        ...
                        <div className="space-y-8">
                            ...
                            {staff.length > 0 && (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {staff.map((member: any) => {
                                        ...
                                        return (
                                            <div key={member.id} className="dealer-glass-card p-6 group relative overflow-hidden bg-[var(--bg-card)] border-[var(--border-default)] hover:translate-y-[-4px] transition-all">
                                                ...
                                                <div className="flex flex-col items-center text-center mb-6">
                                                    <div className="relative mb-4">
                                                        <div className="w-20 h-20 bg-[var(--bg-input)] rounded-3xl flex items-center justify-center border border-[var(--border-default)] group-hover:border-primary/40 transition-colors shadow-2xl relative z-10 overflow-hidden">
                                                            <span className="text-2xl font-black opacity-40">
                                                                {member.user?.firstName?.[0]}{member.user?.lastName?.[0]}
                                                            </span>
                                                            <div className="absolute inset-0 bg-gradient-to-t from-primary/10 to-transparent" />
                                                        </div>
                                                        <div className="absolute -bottom-2 -right-2 p-1.5 bg-[var(--bg-input)] border border-[var(--border-default)] rounded-xl shadow-2xl z-20">
                                                            <ShieldCheck size={14} className="text-emerald-400" />
                                                        </div>
                                                    </div>
                                                    
                                                    <p className="font-black text-lg tracking-tighter group-hover:text-primary transition-colors">{member.user?.firstName} {member.user?.lastName}</p>
                                                    <p className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-widest mt-1">{member.user?.email}</p>
                                                </div>

                                                <div className="space-y-3 pt-4 border-t border-[var(--border-default)]">
                                                    <div className={`flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border border-[var(--border-default)] ${roleConfig.bg} ${roleConfig.color}`}>
                            ...
                            {pendingInvites.length > 0 && (
                                <div className="space-y-4">
                                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-[var(--text-muted)] flex items-center gap-2 px-1">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {pendingInvites.map((invite: any) => {
                                            ...
                                            return (
                                                <div key={invite.id} className="dealer-glass-card p-5 bg-[var(--bg-input)] border-dashed border-[var(--border-default)] flex items-center justify-between group">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-10 h-10 bg-[var(--bg-card)] rounded-xl flex items-center justify-center border border-[var(--border-default)]">
                                                            <Mail size={16} className="text-gray-600" />
                                                        </div>
                                                        <div>
                                                            <p className="text-xs font-black truncate max-w-[140px]">{invite.email}</p>

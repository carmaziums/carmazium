        ...
        <div className="min-h-screen pt-20 pb-12">
            <div className="container mx-auto px-5 flex flex-col lg:flex-row gap-8">
                ...
                <main className="flex-1 space-y-6">
                    <h1 className="text-3xl font-bold font-heading mb-6">Purchase History</h1>
                                    ...
                                    <tbody className="divide-y divide-[var(--border-default)]">
                                        {transactions.map(tx => (
                                            <tr key={tx.id} className="hover:bg-[var(--bg-card)] transition-colors">
                                                <td className="py-3 text-[var(--text-secondary)] font-mono text-sm">{tx.id.slice(0, 8)}</td>
                                                <td className="py-3">
                                                    <div className="flex items-center gap-3">
                                                        {tx.listing?.images?.[0] && (
                                                            <Image src={tx.listing.images[0]} alt="" width={48} height={32} className="w-12 h-8 rounded object-cover bg-[var(--bg-input)]" />
                                                        )}
